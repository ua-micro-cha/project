...
- <a href="https://github.com/DerrickWood/kraken2/blob/master/docs/MANUAL.markdown" target="_blank">kraken2</a>
- <a href="https://hpcdocs.static.arizona.edu/running_jobs/batch_jobs/submitting_jobs" target="_blank">Submitting Batch Jobs</a>

1. Open the slurm script directory in the terminal.

2. Enter code using the `sbatch` command:
```bash
...
Configure a Jupyter Notebook to use an already-existing Conda kraken 2 environment:
...
In the interactive terminal, load the Anaconda module to access `conda`:
``` bash
module load anaconda/2020
```
...
conda activate kraken2-env
```
4. **Install IPython Kernel** in `kraken2-env` to make it available in Jupyter
``` bash
...
9. **Select the New Kernel from the 'Kernel' menu > 'Change kernel'**  
...
Once the IPython kernel has been created for `kraken2-env`, the new kernel becomes a permanent option in the Jupyter Notebook interface.
### How It Works
...
- **Recalling the Kernel**: When creating a new notebook (or opening an existing one), select the “Python (kraken2-env)” kernel from the dropdown Kernel menu to start a notebook in that environment.
...
- **Jupyter Configuration**: Check the Jupyter configuration to ensure it's scanning the correct directories for kernel specifications by running `jupyter kernelspec list` in the terminal. The output will display paths to the kernels available.
***
...
## Bioinformatics Workflow Snapshot
